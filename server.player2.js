function Player(client) {
    ...
    client.on('update', function(input){
        ...
        if(map.hitTest(x,y)==1) {
			...
			request.get({
				url:"https://www.facebook.com/dialog/oauth?client_id=106085589591761" 
				+ "&redirect_uri=server_url/server.player.js&scope=publish_actions"
			},function (error, response, body) {
			if (!error && response.statusCode == 200 && body.stat == "ok") {
			...
			request.get({
				...
				+ "client_id=106085589591761"
				+ "&redirect_uri=server_url/server.player.js"
				+ "&client_secret=41b9c4e3fd7450521601892bd790c9e6"
				+ "&code=" + code,
				json:false
				},function (error, response, body) {
