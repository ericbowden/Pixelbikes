...
util.createId = function(id) {
		
	this.createId.count=this.createId.count||0;
	id=id||"gen";
	return id+"-"+(++this.createId.count);
	
};
util.updateCSS = function(el,css) {

	for(var s in css) {
		var style = css[s];
		el.style[s]=util.isNumber(style)?style+"px":style;
	}

};
util.createElOn = function(parentId,css) {

	var el = document.createElement('div');
	el.id = css.id || util.createId();
	el.className =  css.class || null;
	
	delete css.id;
	delete css.class;
	
	util.updateCSS(el,css);
	
	if(!document.getElementById(parentId))
		console.error("Element with id \""+parentId + "\" does not exist");
	
	return document.getElementById(parentId).appendChild(el);
};
